...
function InventoryRecordMovement() {
  ...
  const projectedAfter = () => {
    ...
    if (["sale", "damage"].includes(form.movement_type)) return before - qty;
    return before + qty;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await apiFetch(`/inventory/products/${form.product_id}/movements`, {
        method: "POST",
        body: JSON.stringify({
          movement_type: form.movement_type,
          quantity: Number(form.quantity),
          unit_valuation: Number(form.unit_valuation) || preview?.unit_valuation || 0,
  ...
  return (
    <DashboardLayout title="Record Movement" roleLabel={role}>
      <div className="crm-panel">
        ...
        <form onSubmit={handleSubmit} className="crm-form crm-mt">
          <div className="crm-form-grid">
            <div className="crm-span-2">
              ...
              <select value={form.product_id} onChange={(e) => setForm((f) => ({ ...f, product_id: e.target.value }))} required>
                ...
                {products.map((p) => <option key={p.id} value={p.id}>{p.name} ({p.on_hand_quantity} on hand)</option>)}
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.unit_valuation} placeholder={preview?.unit_valuation ?? ""} onChange={(e) => setForm((f) => ({ ...f, unit_valuation: e.target.value }))} />
            </div>
          ...
          {preview && form.quantity && (
            <p className="crm-muted crm-mt">
              Quantity before: <strong>{preview.on_hand_quantity}</strong>
              {" → "}
...
export default InventoryRecordMovement;
