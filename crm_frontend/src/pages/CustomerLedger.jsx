...
import { buildIndexParams, formatCurrency, formatDate } from "../utils/customerLedger";
...
function CustomerLedger() {
  ...
  const load = useCallback(() => {
    ...
    apiFetch(`/customer-ledger?${params}`)
      .then(setData)
  ...
  return (
    <DashboardLayout title="Customer Ledger" roleLabel={role}>
      <div className="crm-panel">
        <div className="crm-detail-header">
          <p className="crm-muted">Per-customer invoices, payments, credits, and outstanding balances</p>
          <Link to="/customer-ledger/unassigned" className="crm-btn crm-btn-sm crm-btn-outline">
            Unassigned receivables
          </Link>
        </div>
        ...
        {data?.kpis && (
          <div className="crm-stats-grid crm-mt">
            ...
            <div className="crm-stat-card">
              <p className="crm-stat-label">Customers with balance</p>
              <p className="crm-stat-value">{data.kpis.customers_with_balance}</p>
            </div>
            <div className="crm-stat-card">
              <p className="crm-stat-label">Collected this month</p>
              <p className="crm-stat-value">{formatCurrency(data.kpis.collected_this_month)}</p>
            </div>
        ...
        <div className="crm-filters crm-mt">
          <input
            type="search"
            placeholder="Search customer, GSTIN, email…"
            value={search}
              ...
              <table className="crm-table">
                <thead>
                  <tr>
                    ...
                    <th className="crm-num">Collected</th>
                    <th className="crm-num">Outstanding</th>
                    <th className="crm-num">Overdue</th>
                    <th>Last activity</th>
                    <th className="crm-num">Open inv.</th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
                  {data.items.length === 0 && (
                    <tr><td colSpan={9} className="crm-muted">No customers with ledger activity match your filters.</td></tr>
                  )}
                  {data.items.map((row) => (
                    <tr key={row.contact_id}>
                      ...
                      <td className="crm-num">{formatCurrency(row.total_collected, row.currency)}</td>
                      ...
                      <td className="crm-num">{row.open_invoice_count}</td>
                      <td>
                        <Link to={`/customer-ledger/${row.contact_id}`} className="crm-nav-link">View ledger</Link>
                      </td>
                    </tr>
                  ))}
                </tbody>
